...
//地图网格大小
int laserCloudCenWidth = 10;     //宽
int laserCloudCenHeight = 5;     //高
int laserCloudCenDepth = 10;     //长
...
void transformAssociateToMap()
{
	...
	float srycrx = sbcx*(cblx*cblz*(caly*salz - calz*salx*saly)
				 ...
				 + (caly*salz - calz*salx*saly)*(sbly*sblz + cbly*cblz*sblx) - calx*cblx*cbl     y*saly)
				 + cbcx*sbcy*((caly*calz + salx*saly*salz)*(cbly*cblz + sblx*sbly*sblz)
				 + (caly*salz - calz*salx*saly)*(cbly*sblz - cblz*sblx*sbly) + calx*cblx*sal     y*sbly);
	float crycrx = sbcx*(cblx*sblz*(calz*saly - caly*salx*salz)
				 - cblx*cblz*(saly*salz + caly*calz*salx) + calx*caly*sblx)
				 + cbcx*cbcy*((saly*salz + caly*calz*salx)*(sbly*sblz + cbly*cblz*sblx)
				 + (calz*saly - caly*salx*salz)*(cblz*sbly - cbly*sblx*sblz) + calx*caly*cbl     x*cbly)
				 - cbcx*sbcy*((saly*salz + caly*calz*salx)*(cbly*sblz - cblz*sblx*sbly)
				 + (calz*saly - caly*salx*salz)*(cbly*cblz + sblx*sbly*sblz) - calx*caly*cbl     x*sbly);
	...
	x1 = cos(transformTobeMapped[2]) * transformIncre[3] - sin(transformTobeMapped[2]) * tra     nsformIncre[4];
	y1 = sin(transformTobeMapped[2]) * transformIncre[3] + cos(transformTobeMapped[2]) * tra     nsformIncre[4];
	z1 = transformIncre[5];
...
int main(int argc,char **argv)
{
	...
	tf::StampedTransform atfMappedTrans;
	aftMappedTrans.frame_id_ = "/camera_init";
	aftMappedTrans.child_frame_id_ = "/aft_mapped";


	std::vector<int> pointSearchInd;
	std::vector<int> pointSearchSqDis;
	...
	while(status)
	{
		...
		if(newLaserCloudCornerLast && newLaserCloudSurfLast && newLaserCloudFullRes && newLaserOdometry && fabs(timeLaserCloudCornerLast - timeLaserOdometry) < 0.005 && fabs(timeLaserCloudSurfLast - timeLaserOdometry) < 0.005 && fabs(timeLaserCloudFullRes - timeLaserOdometry) < 0.005)
		{
			...
			if(frameCount >= stackFrameNum)
			{
				...
				pointOnYAxis.y = 10.0;
				pointOnYAxis.z = 0.0;
				pointAccociateToMap(&pointOnYAxis, &pointOnYAxis);
				...
				while(centerCubeI < 3)
				{
					for(int j = 0; j < laserCloudHeight; j ++)
					{
						for(int k = 0; k < laserCloudDepth; k ++)
						{
							...
							pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudCubeCornerPointer = laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k];
							pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudCubeSurfPointer = laserCloudSurfArray[i + laserCloudWidth * j + lasercloudWidth * laserCloudHeight * k];
				...
				while(centerCubeI >= laserCloudWidth - 3)
				{
					for(int j = 0; j < laserCloudHeight; j ++)
					{
						for(int k = 0; k < laserCloudDepth; k ++)
						{
							int i = 0;
							pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudCubeCornerPointer = laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k];
							pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudCubeSurfPointer = laserCloudSurfArray[i + laserCloudWidth * j + laserCloudWidth * laserCloufHeight * k];
							...
							laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudWidth * laserCloudHeight * k] = laserCloudCubeCornerPointer;
							laserCloudSurfArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k] = laserCloudCubeSurfPointer;
				...
				while(centerCubeJ >= laserCloudHeight - 3)
				{
					for(int i = 0; i < laserCloudWidth; i ++)
					{
						...
						{
							int j = 0;
							pcl::PointCloud<pcl::PointXYZI>::Ptr laserlCloudCubeCornerPointer = laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k];
							pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudCubeSurfPointer = laserCloudSurfArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k];
				...
				while(centerCubeK >= laserCloudDepth - 3)
				{
					for(int i = 0; i < laserCloudWidth; i ++)
					{
						for(int j = 0; j < laserCloudHeight; j ++)
						{
							int k = 0;
							pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudCubeCornerPointer = laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k];
							pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudSurfPointer = laserCloudSurfArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k];

							for(; k < laserCloudDepth - 1; k ++)
							{
								laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k] = laserCloudCorenrArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * (k + 1)];
								laserCloudSurfArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k] = laserCloudSurfArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * (k + 1)];
							}
				...
				for(int i = centerCubeI - 2; i <= centerCubeI + 2; i ++)
				{
					for(int j = centerCubeJ - 2; j < centerCubeJ + 2; j ++)
					{
						...
						{
							if(i >= 0 && i < laserCloudWidth && j >= 0 && j < laserHeight && k >= 0 && k < laserCloudDepth)
							{
				...
				if(laserCloudCornerFromMapNum > 10 && laserCloudSurfFromMapNum > 100)
				{
					...
					for(int iterCount = 0; iterCount < 10; iterCount ++)
					{
						...
						for(int i = 0; i < laserCloudCornerStackNum; i ++)
						{
							...
							if(pointSearchSqDis[4] < 1.0)
							{
								...
								for(int j = 0; j < 5; j ++)
								{
									...
									float az = laserCloudCornerFromMap->points[pointSearchIndj[]].z - cz;
								...
								if(matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1))
								{
									...
									float z1 = cz + 0.2 * matV1.at<float>(0, 2);
						...
						float deltaT = sqrt(pow(rad2deg(matX.at<float>(3, 0)), 2) +
											pow(rad2deg(matX.at<float>(4, 0)), 2) +
											pow(rad2deg(matX.at<float>(5, 0)), 2));
